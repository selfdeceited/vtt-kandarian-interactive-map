...
function normaliseMarker(m: unknown): Location {
  ...
  return {
    ...
    type: (["settlement", "other"].includes(loc.type as string)
      ? loc.type
      : "other") as LocationType,
  };
}

const SILO_UUID = import.meta.env.VITE_SILO_UUID;
...
async function fetchStore(): Promise<MapStore[]> {
  ...
  return (data as MapStore[]).map((s) => ({
    ...s,
    markers: s.markers.map(normaliseMarker),
  }));
}
