...
- You need the marker storage. I use [JsonSilo](https://jsonsilo.com/) with `VITE_SILO_JSON_KEY` and `SILO_UUID` in your `.env.local` file. Both the UUID and the manage key is available from their console, but beware that `VITE_SILO_JSON_KEY` has the expiration date.
